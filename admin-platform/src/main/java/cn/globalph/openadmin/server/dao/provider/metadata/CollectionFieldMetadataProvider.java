...
import cn.globalph.common.presentation.override.AdminPresentationCollectionOverride;
...
public class CollectionFieldMetadataProvider extends AdvancedCollectionFieldMetadataProvider {
    ...
    protected FieldMetadataOverride overrideCollectionMergeMetadata(AdminPresentationMergeOverride merge) {
        ...
        for (Map.Entry<String, AdminPresentationMergeEntry> entry : overrideValues.entrySet()) {
            ...
            if (entry.getKey().equals(PropertyType.AdminPresentationCollection.ADDTYPE)) {
                fieldMetadataOverride.setAddType(OperationScope.valueOf(stringValue));
    ...
    protected void buildCollectionMetadata(Class<?> parentClass, Class<?> targetClass, Map<String, FieldMetadata> attributes, FieldInfo field, FieldMetadataOverride collectionMetadata) {
        ...
        cn.globalph.openadmin.dto.PersistenceOperationType dtoOperationTypes = new cn.globalph.openadmin.dto.PersistenceOperationType(OperationScope.BASIC, OperationScope.BASIC, OperationScope.BASIC, OperationScope.BASIC, OperationScope.BASIC);
        ...
        if (AddMethodType.LOOKUP == metadata.getAddMethodType()) {
            dtoOperationTypes.setRemoveType(OperationScope.NONDESTRUCTIVEREMOVE);
        }
        ...
        if (serverMetadata != null) {
            foreignKeyName = ((ForeignKey) serverMetadata.getPersistenceAssociation().getPersistenceAssociationItems
                    ().get(PersistenceAssociationItemType.FOREIGNKEY)).getManyToField();
        }
        ...
        if (serverMetadata != null) {
            ForeignKey foreignKey = (ForeignKey) metadata.getPersistenceAssociation().getPersistenceAssociationItems().get(PersistenceAssociationItemType.FOREIGNKEY);
