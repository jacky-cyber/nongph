...
var Class = {
	...
	create:function(_name,_super,_interfaces,defination) {
		var classDefineObj = {
			className:_name,
			_superClass : this._getSuper(_super),
			_interfaces : this._getInterfaces(_interfaces, _super),
			_define : this._getDefination(defination, _interfaces, _super)
		};
	...
	_getInterfaces : function(arg1, arg2) {
		if (arg1 && arg1 instanceof Array) {
			return arg1;
		}

		if (arg2 && arg2 instanceof Array) {
			return arg2;
		}
	...
	_getDefination : function(arg1, arg2, arg3) {
		if (arg1 && typeof arg1 == "object") {
			return arg1;
		}

		if (arg2 && typeof arg2 == "object" && !(arg2 instanceof Array)) {
			return arg2;
		}

		if (arg3 && typeof arg3 == "object" && !(arg3 instanceof Array)) {
			return arg3;
		}

		//throw new Error("Could not get the class defination");
		return {};
	},
...
var DOM={
	...
	/**
	 * Add the class name of style to element
	 *
	...
	/**
	 * Remove the class name of style from element
	 *
	 * @param {Node} elem the element
	 * @param {String} name the style (class) name
	 *        {Array<String>} the class name list
	 */
	removeStyleName : function(elem, name) {
		var classList = elem.className.split(/[ ]+/);
		if (name instanceof Array) {
			for (var i=0; i<name.length; i++) {
				classList.remove(name[i]);
			}
		} else {
...
Class.create("Element" ,null ,null ,{
	...
	addStyleName : function(name) {
		DOM.addStyleName(this.getDom(), name);
	},
	
	appendChild : function(elem) {
		var child = elem.getDom ? elem.getDom() : elem;
		this.getDom().appendChild(child);
	},
	
	removeChild : function(elem) {
		var child = elem.getDom ? elem.getDom() : elem;
		this.getDom().removeChild(child);
	},
	
	cloneNode : function(deep) {
		return new Element(this.getDom().cloneNode(deep));
	},
	
	get : function(attrName) {
		return this.getDom()[attrName];
	},
	
	set : function(attr, v) {
		return this.getDom()[attr] = v;
	},
	...
	removeStyleName : function(name) {
		DOM.removeStyleName(this.getDom(), name);
	},
	
	setInnerHTML : function(str) {
		return this.set("innerHTML", str);
	},
	
	setTextContent : function(str) {
		return this.set(DOM.browser.isMoz ? "textContent" : "innerText", str);
	}
...
var Console={
	...
	_board: null,
	init: function() {
		this._board = new Element("div", {id: "consoleWrapper"});
		$doc.body.appendChild(this._board.getDom());
		this.on = true;
	},
	error : function(obj) {
		this._queue.push({level: this.ERROR, message:obj});
		this._put();
	},
	put : function(str) {
		this._queue.push({level:this.INFO, message:str});
		this._put();
	},
	...
	_put:function(str) {
		...
		while (this._queue.length > 0) {
			...
			var time = Format.formatDate(new Date(),Format.StandardTime)+" ";
			var _elem;
			if (o.level == this.INFO) {
				_elem = new Element("div");
				_elem.setTextContent(time + o.message);
			} else if (o.level == this.ERROR) {
				_elem = new Element("div", {"class": "errorItem"});
				if (typeof o.message!="object") {
					_elem.setTextContent(time + o.message);
				} else {
					_elem.setInnerHTML("Err Name: "+o.message.name+"<br/>Err Msg: "+o.message.message);
				}
			}
			if (_elem) _board.appendChild(_elem);
		}
		/*
		...
		_board.set("scrollTop", _board.get("scrollHeight") - _board.get("clientHeight"));
		_board=null;
	},
	clear:function() {
		delete this._board;
...
Console.info = Console.put;

Window.addLoadListener({
	onLoad:function() {
		Console.init();
		Console.put(new Date());
		// Console.info(Object.toJSON(DOM.browser));
	}
});
Window.addCloseListener({
	onClose:function() {
...
Class.create("ui.Calendar",null,null,{
	$tatic : {
		SOURCE : "Widgets/Calendar/gw_calendar.htm",
		_sink : [],
		destroy :function() {
			var _a;
...
Class.create("ecp.ui.impl.GenerateXMLVisitor" ,"msg.ui.impl.GenerateXMLVisitor" ,null ,{
	...
	visitCheckListPattern : function(container) {
		...
		for (var i = 0, j = container.node.maxOccurs; i<j; i++) {
			if (values[i]) {
				xml.push('<'+container.name+'>'+values[i]+'</'+container.name+'>');
			}
		}
		return xml.join('');
	},
	...
	visitComplexTextPattern : function(container) {
		var xml = [], _attrs;
		if (container.enabled) {
			xml.push('<'+container.name);
			_attrs = container._attrs;
			for (var i in _attrs) {
				var attr = _attrs[i];
				if (attr && i==attr.name) xml.push(' '+attr.name+'="'+attr.getValue()+'"');
			}
			xml.push('>'+container.getValue()+'</'+container.name+'>');
		}
		return xml.join('');
	},
	...
	visitMultilinePattern : function(container) {
		...
		var xml = [];
		for (var i=0,j=v.length; i<j; i++) {
			xml.push('<'+container.name+'>'+v[i]+'</'+container.name+'>');
		}
		return xml.join('');
	}
});var PATH;
...
Object.extend(PATH,{
	...
	//GET_CURRENCY_EXPANSION : "/powerx/CurrencyValidateServlet",
...
//var MsgMgr = Class.get("csp.ui.MessageManafinOrXmlger");
...
function loadCamtMessage(id, lang, readOnly) {
	var builder = new (Class.get("msg.ui.MessageBuilder"))({
		...
		createMessage : function() {
			...
			fillCreator();
			this.fillMessage();
			if($win.onUiLoad!=null)
			{
				$win.onUiLoad(currentDocument);
			}
...
$win.searchContainer = function(elem) {
	var t = $('search').value;
	if (currentDocument) 
		currentDocument.getMessage().showMatch(t);
}
