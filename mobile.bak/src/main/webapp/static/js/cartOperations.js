$(function () {
	...
	$("input[type='sumbit']").on("click",function(){
		HC.mask();
    });
    ...
    $('body').on('click', 'a.addCart,input.addToCart,input.addToWishlist', function() {
    	HC.mask();
        var $button = $(this),
                    	 ...
                    	 HC.unmask();
		                     if (data.error) {
		                        if (data.error == 'allOptionsRequired') {
                                    ...
                                    HC.showNotification("暂时缺货", 7000);
		                        } else {
                                    HC.showNotification("系统出错");
		                                }
		                     } else {
		                        ...
		                        if (modalClick) {
		                            ...
		                            showInCartButton(data.skuId, 'wishlist');
		                        } else {
		                        ...
		                        if (wishlistAdd) {
		                            HC.showNotification(data.skuName + "  已加入收藏！");
		                        } else {
//		                            HC.showNotification(data.skuName + "  已加入购物车！", 2000);
		                        	HC.showNotification("添加成功", 2000);
		                                }
		                          }
		                    }
                   );
...
//    
    ...
    $('.quantity-increase').click(function () {
        var form = $(this).closest('form');
        var quantity = parseInt(form.children('input.quantity').val());
        if(!(quantity==99)){
        	quantity++;
        }
        form.children('input.quantity').val(quantity);
        HC.mask();
        form.submit();
    });
    ...
    $('.quantity-decrease').click(function () {
        ...
        var quantity = parseInt(form.children('input.quantity').val());
        if(quantity>1){
        	form.children('input.quantity').val(quantity-1);
        }
        HC.mask();
        form.submit();
    });
