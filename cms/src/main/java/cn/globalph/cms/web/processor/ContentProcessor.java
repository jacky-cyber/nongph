...
import cn.globalph.cms.web.deeplink.ContentDeepLinkServiceImpl;
...
/**
 * Processor used to display structured content that is maintained with the globalph CMS.
 *
...
public class ContentProcessor extends AbstractModelVariableModifierProcessor {
    ...
    protected void modifyModelAttributes(final Arguments arguments, Element element) {        
        ...
        contentItems = getContentItems(contentName, maxResults, request, mvelParameters, currentSandbox, structuredContentType, arguments, element);
        ...
        if (StringUtils.isNotBlank(deepLinksVar) && contentItems.size() > 0 ) {
            ...
            extensionManager.getHandlerProxy().addExtensionFieldDeepLink(links, arguments, element);
            extensionManager.getHandlerProxy().postProcessDeepLinks(links);
    ...
    protected List<StructuredContentDTO> getContentItems(String contentName, Integer maxResults, HttpServletRequest request,
        ...
        extensionManager.getHandlerProxy().addAdditionalFieldsToModel(arguments, element);
