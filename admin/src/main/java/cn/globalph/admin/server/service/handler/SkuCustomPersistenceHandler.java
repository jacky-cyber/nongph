...
public class SkuCustomPersistenceHandler extends CustomPersistenceOperationHandlerAdapter {
    ...
    public Boolean canHandleInspect(PersistencePackage persistencePackage) {
        return canHandle(persistencePackage, persistencePackage.getPersistenceAssociation().getPersistenceOperationType()
                .getInspectType());
    }

    @Override
    public Boolean canHandleFetch(PersistencePackage persistencePackage) {
        OperationScope fetchType = persistencePackage.getPersistenceAssociation().getPersistenceOperationType().getFetchType();
        return canHandle(persistencePackage, fetchType);
    }

    @Override
    public Boolean canHandleAdd(PersistencePackage persistencePackage) {
        OperationScope addType = persistencePackage.getPersistenceAssociation().getPersistenceOperationType().getAddType();
        return canHandle(persistencePackage, addType);
    }

    @Override
    public Boolean canHandleUpdate(PersistencePackage persistencePackage) {
        OperationScope updateType = persistencePackage.getPersistenceAssociation().getPersistenceOperationType().getUpdateType();
     ...
     * 
     */
    protected Boolean canHandle(PersistencePackage persistencePackage, OperationScope operationType) {
        String ceilingEntityClassname = persistencePackage.getRootEntityClassname();
        try {
            Class testClass = Class.forName(ceilingEntityClassname);
            return Sku.class.isAssignableFrom(testClass) && 
                    //ArrayUtils.isEmpty(persistencePackage.getCustomCriteria()) &&
                    OperationScope.BASIC.equals(operationType) && 
                    (persistencePackage.getPersistenceAssociation().getPersistenceAssociationItems().get(PersistenceAssociationItemType.ADORNEDTARGETLIST) == null);
    ...
    public DynamicResultSet inspect(PersistencePackage persistencePackage, DynamicEntityDao dynamicEntityDao, InspectHelper helper) throws ServiceException {
        try {
            ...
            adornedPersistenceModule.setPersistenceManager((PersistenceOperationExecutor)helper);
            ...
            ServiceException ex = new ServiceException("Unable to retrieve inspection results for " +
                    persistencePackage.getRootEntityClassname(), e);
            throw ex;
        }
    }
    ...
    public DynamicResultSet fetch(PersistencePackage persistencePackage, CriteriaTransferObject cto, DynamicEntityDao dynamicEntityDao, RecordHelper helper) throws ServiceException {
        ...
        try {
            ...
            List<FilterMapping> filterMappings = helper.getFilterMappings(persistencePerspective, cto, ceilingEntityClassname, originalProps);
            ...
            List<Serializable> records = helper.getPersistentRecords(persistencePackage.getRootEntityClassname(), filterMappings, cto.getFirstResult(), cto.getMaxResults());
            //Convert Skus into the client-side Entity representation
            Entity[] payload = helper.getRecords(originalProps, records);

            int totalRecords = helper.getTotalRecords(persistencePackage.getRootEntityClassname(), filterMappings);
            ...
            throw new ServiceException("Unable to perform fetch for entity: " + ceilingEntityClassname, e);
        }
    }
