...
Ext.ux.UploadDialog.FileRecord = Ext.data.Record.create([{name:'fileId',type:'int'},{
			name : "filename"
		}, {name : 'filepath'},{
			name : "state",
			type : "int"
...
Ext.extend(Ext.ux.UploadDialog.Dialog, Ext.Window, {
	...
	fillToolbar : function() {
		...
		tb.add("-");
		tb.x_buttons.indicator = tb.addItem(new Ext.Toolbar.Item(Ext.DomHelper.append(tb.getEl(),{id:'x-button-indicator',tag : "div",cls : "ext-ux-uploaddialog-indicator-stoped",html : "&#160"})));
//		tb.addItem(new Ext.Toolbar.Item(Ext.DomHelper
//		.append(tb.getEl(), {
...
//					.child(tb.x_buttons.upload.buttonSelector).dom[tb.x_buttons.upload.tooltipType] = this.i18n.upload_btn_start_tip;
	...
	onCloseButtonClick : function() {
		...
		for(var i=0;i<store.getCount();i++){
			...
			if(record.data.state==Ext.ux.UploadDialog.FileRecord.STATE_FINISHED){//上传完成后的才显示出来
				fileArr.push(record.data);
			}
		}
		
		if(this.callback!=null){
			this.callback.call(this,fileArr);
		}
		this.close();
	...
	onAjaxSuccess : function(response, options) {
		...
		try {
			//var rsp=response.responseText.replace('<pre>','').replace('</pre>','');
			var rt = response.responseText;
	        var filter = rt.match(/^<[^>]+>((?:.|\n)*)<\/[^>]+>$/);
	        if (filter) {
	            rt = filter[1];
	        }
			var json_response = Ext.util.JSON.decode(rt);
			options.record.data.filename=json_response.fileName;
			options.record.data.fileId=json_response.fileId;
			options.record.data.filepath=json_response.filePath;
