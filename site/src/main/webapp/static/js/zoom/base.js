	...
	$.fn.jdTab=function(option,callback){
		...
		var autoSwitch=function(){
			...
			tabItem.each(function(n){
				$(this).bind( s.event, function(){ clearTimeout(s.subTimer);
...
(function($){
	$.fn.jdSlide=function(option){
		var settings=$.extend( { width:null,
			                     height:null,
			                     pics:[],
			                     index:0,
			                     type:"num",
		...
		var initIndex=function(){
			...
			for(var i=0;i<total;i++){
				...
				switch(settings.type){
					case "num":x=i+1;
						break;
					case "string":x=settings.pics[i].alt;break;
					case "image":x="<img src='"+settings.pics[i].breviary+"' />";
					default: break;
				};
...
mark=function(sku,type){
	$.getJSON(calluri,{sku:sku,type:type},callback1);
	log(1, type, sku);
};
function search(id){
...
function compare(){
	if($("#compare").get(0)==null){
		$("body").append("<div id=\"compare\" class=\"compare\">" +
				         	"<div class=\"mt\"><h5>��Ʒ�Ƚ�</h5>" +
				         		"<div class=\"extra\" onclick=\"clearCompare()\"></div>" +
				         	"</div>" +
				         	"<div class=\"comPro\">" +
				         		"<ul class=\"mc\" id=\"comProlist\"></ul>" +
				         		"<div class=\"mb\">" +
				         			"<input type=\"button\" value=\"�Ա���ѡ��Ʒ\" class=\"btn\" id=\"compareImg\" onclick=\"openCompare()\">" +
				         		"</div>" +
				         	"</div>" +
				         "</div>");
