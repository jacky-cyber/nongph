...
import cn.globalph.openadmin.web.form.entity.EntityFormValidator;
import cn.globalph.openadmin.web.form.entity.Field;
import cn.globalph.openadmin.web.form.entity.FieldGroup;
 ...
 *
 * @see cn.globalph.openadmin.web.handler.AdminNavigationHandlerMapping
 * @author felix.wu
 */
public abstract class BasicAdminController extends BasicController {
    ...
    protected EntityAdminService entityAdminService;

    @Resource(name = "blFormBuilderService")
    protected DynamicFormBuilder formBuilder;
    ...
    protected ListGrid getCollectionListGrid(ClassMetadata mainMetadata, Entity entity, Property collectionProperty,
            ...
            throws ServiceException {
        String idProperty = mainMetadata.getIdProperty();
        ...
        DynamicResultSet drs = entityAdminService.getRecordsForCollection(mainMetadata, entity, collectionProperty,
                getCriteria(requestParams), getStartIndex(requestParams), getMaxIndex(requestParams), sectionCrumbs).getDynamicResultSet();

        ListGrid listGrid = formBuilder.buildCollectionListGrid(entity.findProperty(idProperty).getValue(), drs,
    ...
    protected EntityForm getBlankDynamicFieldTemplateForm(DynamicEntityFormInfo info, EntityForm dynamicFormOverride) 
        ...
        PersistencePackageRequest ppr = PersistencePackageRequest.standard()
                .withRootEntityClassname(info.getCeilingClassName())
        ...
        EntityForm dynamicForm = formBuilder.createEntityForm(cmd, null);
    ...
    protected EntityForm getDynamicFieldTemplateForm(DynamicEntityFormInfo info, String entityId, EntityForm dynamicFormOverride) 
        ...
        // However, when we fetch, the second custom criteria needs to be the id
                ...
                Entity entity = entityAdminService.getRecord(ppr, entityId, cmd, true).getDynamicResultSet().getRecords()[0];
        ...
        EntityForm dynamicForm = formBuilder.createEntityForm(cmd, entity, null, null);
        
        for (Field field : fieldsToMove) {
            FieldMetadata fmd = cmd.getPMap().get(field.getName()).getFieldMetadata();
    ...
    protected void extractDynamicFormFields(EntityForm entityForm) {
        ...
        for (Entry<String, Field> entry : dynamicFields.entrySet()) {
            ...
            if (dynamicForm == null) {
                ...
                dynamicForm.setRootEntityClassname(info.getCeilingClassName());
    ...
    /**
     * <p>Helper method to return an array of {@link cn.globalph.openadmin.dto.FilterAndSortCriteria} based on a map of propertyName -> list of criteria
     ...
     * @return the final array of {@link cn.globalph.openadmin.dto.FilterAndSortCriteria} to pass to the fetch
       * 
    ...
    protected FilterAndSortCriteria[] getCriteria(Map<String, List<String>> requestParams) {
        ...
        if (CollectionUtils.isNotEmpty(sortProperties)) {
            ...
            for (int i = 0; i < sortProperties.size(); i++) {
            	boolean sortAscending = SortDirection.ASCENDING.name().equals(sortDirections.get(i));
            	
            	String prop = sortProperties.get(i);
                FilterAndSortCriteria propertyCriteria = fasMap.get( prop );
                ...
                if (propertyCriteria != null) {
                    ...
                    FilterAndSortCriteria fasc = new FilterAndSortCriteria( prop );
    ...
    protected String getClassNameForSection(String sectionKey) {
        ...
        extensionManager.getHandlerProxy().overrideClassNameForSection(erh, sectionKey, section);
        if (erh.getContextMap().get(BaseAdminControllerExtensionHandler.NEW_CLASS_NAME) != null) {
            return (String) erh.getContextMap().get(BaseAdminControllerExtensionHandler.NEW_CLASS_NAME); 
           }
    ...
    protected void setModelAttributes(Model model, String sectionKey) {
        ...
        extensionManager.getHandlerProxy().setAdditionalModelAttributes(model, sectionKey);
    }
    ...
    protected PersistencePackageRequest getSectionPersistencePackageRequest(String sectionClassName, 
        ...
        PersistencePackageRequest ppr = PersistencePackageRequest.standard()
                .withRootEntityClassname(sectionClassName)
