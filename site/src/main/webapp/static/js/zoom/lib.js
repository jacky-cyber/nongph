...
new function(settings){
	...
	queryObject.prototype={queryObject:true,
...
$.login=function(options){
	options=$.extend({loginService:"http://passport.360buy.com/loginservice.aspx?callback=?",
		              loginMethod:"Login",
	...
	if(options.loginService!="" && options.loginMethod!=""){
		$.getJSON(options.loginService, {method:options.loginMethod}, function(result){
					if(result!=null){
...
jQuery.fn.pagination=function(maxentries,opts){
	opts=jQuery.extend( { items_per_page:10,
...
function getBrandService(json){
	...
	$.each(data,function(i){
		...
		if (d1.length>0){
			...
			html+="<div class='norecode'>无推荐品牌</div>";
		}
		html+="</dd></dl><dl class=\"promotion\"><dt>促销</dt><dd><ul>";
		if (d2.length>0){
			$.each(d2,function(k){
				html+="<li>��<a href=\""+ d2[k]["u"] +"\">"+ d2[k]["n"] +"</a></li>";
			});
		}else{
			html+="<div class='norecode'>无促销</div>";
		}
		html+="</ul></dd></dl>";
...
(function($){
	$.fn.jdPosition=function(option){
		...
		switch(s.mode){
			default:case "center":var ow=$(this).outerWidth(),oh=$(this).outerHeight();
			                      var w=$.browser.isMinW(ow),h=$.browser.isMinH(oh);
			                      $(this).css( {left:(!$.browser.isIE6)?(w?0:Math.max(($.browser.client().width-ow)/2,0)+"px"):(w?0:Math.max($.browser.scroll().left+($.browser.client().width-ow)/2,0)+"px"),
...
(function($){
	$.fn.jdThickBox=function(option,callback){
		...
		var inputData=function(element){
			...
			switch(s.type){
				default:case "text": $(".thickcon").html(s.source);$(".thickcon").removeClass(s._loading);
				                     break;
				case "html": $(s.source).clone().appendTo($(".thickcon")).show();
				             $(".thickcon").removeClass(s._loading);
				             break;
				case "image":s._index=(s._index==null)?object.index(element):s._index;
				             $(".thickcon").append("<img src='"+s.source+"' width='"+s.width+"' height='"+s.height+"'>");
				             s.source=null;
				             $(".thickcon").removeClass(s._loading);
				             break;
				case "ajax":
				case "json":if(callback){
								callback(s.source,$(".thickcon"),function(){
										$(".thickcon").removeClass(s._loading);
									});
							};
							break;
			    case "iframe":$("<iframe src='"+s.source+"' marginwidth='0' marginheight='0' frameborder='0' scrolling='no' style='width:"+s.width+"px;height:"+s.height+"px;border:0;'></iframe>").appendTo($(".thickcon"));
			    			  $(".thickcon").removeClass(s._loading);
			                  break;
		    };
		};
		var reposi=function(){
			...
			var h1=(s._titleOn?$(".thicktitle").outerHeight():0)+$(".thickcon").outerHeight();
			...
			if($.browser.isIE6){
				...
				$(".thickframe").add(".thickdiv").css( { width:w2?ow:"100%",
...
(function($){
	$.fn.jdMarquee=function(option,callback){
		...
		var init=function(){
			...
			object.after(clone);
			switch(s.deriction){
				default:case "up": object.css({marginLeft:0,marginTop:0});
								   clone.css({marginLeft:0,marginTop:height+"px"});
								   s.pos.clone=height;
								   break;
				case "down": object.css({marginLeft:0,marginTop:0});
				             clone.css({marginLeft:0,marginTop:-height+"px"});
				             s.pos.clone=-height;
				             break;
				case "left": object.css({marginTop:0,marginLeft:0});
				             clone.css({marginTop:0,marginLeft:width+"px"});
				             s.pos.clone=width;
				             break;
				case "right":object.css({marginTop:0,marginLeft:0});
				             clone.css({marginTop:0,marginLeft:-width+"px"});
				             s.pos.clone=-width;
				             break;
			};
			if(s.auto){
				initMainTimer();
				object.hover(function(){clear(s.mainTimer);},function(){initMainTimer();});
				clone.hover(function(){clear(s.mainTimer);},function(){initMainTimer();});
			};
			if(callback){
				callback();
			};
		...
		var initControls=function(){
			if(s._front!=null){
				$(s._front).click( function(){ $(s._front).addClass(s.disabled);
                           disControl(true);
                           clear(s.mainTimer);
                           s.convert=true;
                           s.btn="front";
			...
			if(s._continue!=null){
				$(s._continue).click(function(){initMainTimer();});
			}
		};
...
var Properties={ "_action":"http://search.360buy.com/ks?keyword=",
		         "_input":"key",
		         "_div":"tie",
		         "_script_id":"_rs",
		         "_htmls":new Array("<li onmouseover=\"$o.over(this)\" onclick=\"$o.selectText(this)\" onmouseout=\"$o.out(this)\" title='","' id='d_","'><span>Լ","��</span><div>","</div></li>",""),
...
(function($){
	$.jdCalcul=function(pids){
		...
		var init=function(data){
			var s=$.extend({contentid:"#limit",
				            clockid:"#clock",
				            rankid:"#rank",
				            limitid:"#limitbuy"},data||{});
			...
			var createHtml=function(){
				var html="<li><div class=\"p-img\"><a href=\"{6}{0}.html\" target=\"_blank\"><img src=\"{1}\" width=\"100\" height=\"100\" /></a>{2}</div><div class=\"p-name\"><a href=\"{6}{0}.html\" target=\"_blank\">{3}</a></div><div class=\"p-price\">�����ۣ�<strong>{4}</strong>{5}</div></li>";
				var html1="<ul>";
				$.each(s.pros,function(i){
					var id=s.pros[i].id,tp=s.pros[i].tp,zt=(s.pros[i].zt==1)?"<div class='pi9'></div>":"<div class='pi10'></div>",mc=unescape(s.pros[i].mc),qg=s.pros[i].qg,zk="("+s.pros[i].zk+"��)";
			...
			var run=function(){
				if(ST>0){
					...
					if(ET>0){
						...
						S=(ET-H*3600)%60;
						s.clockid.html("ʣ��<b>"+H+"</b>Сʱ<b>"+M+"</b>��<b>"+S+"</b>��");
						ET--;
					}else{
						s.clockid.html("��������");
						clearInterval(timer);
			...
			run();
			timer=setInterval(function(){run()},1000);
		};
...
/* getrecent */
var jdRecent={element:$("#recent ul"),
		      ...
		      init:function(){
		    	  ...
		    	  $.cookie(this.cookiename,this.list,{expires:7,path:"/",domain:"360buy.com",secure:false});
		    	  this.getData(this.list);
		      },
		      clear:function(){
		    	  $.cookie(this.cookiename,"",{expires:7,path:"/",domain:"360buy.com",secure:false});
		      },
...
$("#clearRec").click(function(){jdRecent.clear();jdRecent.getData(0);});
/*showTip2*/
var display2=false;
...
//暂时取消JD的购物车状态更新方案，以后考虑在现有方案中权衡选择
//refreshMiniCart();
$(function(){
