 ...
 */
public class GlobalphThymeleafViewResolver extends ThymeleafViewResolver {
    private static final Log LOG = LogFactory.getLog(GlobalphThymeleafViewResolver.class);
    ...
    protected View loadAjaxRedirectView(String redirectUrl, final Locale locale) throws Exception {
        if (isAjaxRequest()) {
            ...
            addStaticVariable(ControllerUtility.BLC_REDIRECT_ATTRIBUTE, redirectUrl);
    ...
    protected View loadView(final String originalViewName, final Locale locale) throws Exception {
        ...
        extensionManager.getHandlerProxy().overrideView(erh);
    ...
    protected boolean isAjaxRequest() {
        ...
        if (WebRequestContext.getWebRequestContext() != null) {
            HttpServletRequest brcRequest = WebRequestContext.getWebRequestContext().getRequest();
            if (brcRequest != null) {
                request = brcRequest;
            }
        }
